...
const { sendCertificateEmail, sendFeedbackRequestEmail } = require("../utils/emailService");

function getDerivedEventStatus(event) {
  if (event.status === "completed" || event.status === "deleted") {
    return event.status;
  }
  ...
  const threeDaysBeforeStart = startDate
  ...
  if (event.status === "upcoming" && threeDaysBeforeStart && now >= threeDaysBeforeStart) {
    return "published";
  }
...
exports.createEvent = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      `INSERT INTO events
      (title, description, venue, city, start_date, end_date, category, type, organizer, email, address, maplink, registration_limit, registration_deadline, banner_image, status, parking_enabled, standard_slots, premium_slots, valet_slots, standard_price, premium_price, valet_price)
  ...
  try {
    ...
    const result = await pool.query(
      ...
      `SELECT 
        ...
        events.banner_image,
        events.status,
        events.parking_enabled,
        events.standard_slots,
        events.premium_slots,
        events.valet_slots,
        events.standard_price,
        events.premium_price,
        events.valet_price,
       ...
       GROUP BY events.id, events.title, events.description, events.venue, events.city, events.start_date, events.end_date, events.category, events.type, events.organizer, events.email, events.address, events.maplink, events.registration_limit, events.registration_deadline, events.banner_image, events.status, events.parking_enabled, events.standard_slots, events.premium_slots, events.valet_slots, events.standard_price, events.premium_price, events.valet_price
       ORDER BY events.id DESC`

    );

    // Process each event to keep its lifecycle status in sync with current time.
    const events = result.rows.map(event => {
      const finalStatus = getDerivedEventStatus(event);

      if (finalStatus !== event.status) {
        console.log(`🔄 Auto-updating event ${event.id} status from ${event.status} to ${finalStatus}`);

        pool.query(
          `UPDATE events SET status = $1 WHERE id = $2`,
          [finalStatus, event.id]
        ).catch(err => {
          console.error(`❌ Failed to auto-update event ${event.id} status:`, err);
        });
      }
...
exports.endEvent = async (req, res) => {
  ...
  try {
    ...
    if (event.status === 'completed') {
      return res.json({
        message: "Event is already completed. You can now upload media for this event.",
        eventId: Number(id),
        status: "completed",
    ...
    res.json({
      message: `Event marked as completed. Certificate and feedback email delivery has started for ${registrations.length} registered participants. You can now upload media for this event.`,
      eventId: Number(id),
      status: "completed",
      certificateSummary: {
    ...
    setImmediate(async () => {
      ...
      const sentCount = certificateResults.filter(
        (result) => result.status === "fulfilled" && result.value?.success
      ).length;
      const failedCount = certificateResults.length - sentCount;

      console.log(
        `Certificate delivery completed for event ${id}: ${sentCount} sent, ${failedCount} failed`
      );
      ...
      try {
        ...
        const feedbackJobs = Array.from(uniqueRecipients.values()).map((recipient) => {
          const feedbackLink = `${frontendBaseUrl}/feedback-form/${event.id}?email=${encodeURIComponent(recipient.email)}`;
        ...
        const feedbackResults = await Promise.allSettled(feedbackJobs);
        const feedbackSent = feedbackResults.filter(
          (result) => result.status === "fulfilled" && result.value?.success
        ).length;
        const feedbackFailed = feedbackResults.length - feedbackSent;

        console.log(
          `Feedback invite delivery completed for event ${id}: ${feedbackSent} sent, ${feedbackFailed} failed`
        );
      } catch (feedbackError) {
        console.error(`Failed to send feedback invites for event ${id}:`, feedbackError);
      }
    });
...
async function generateAndSendCertificate(event, registration) {
  ...
  return new Promise((resolve, reject) => {
    doc.on('end', async () => {
      ...
      try {
        const emailResult = await sendCertificateEmail(registration.email, registration.name, event.title, pdfBuffer);

        if (!emailResult?.success) {
          return reject(new Error(emailResult?.error || `Certificate email failed for ${registration.email}`));
        }
...
exports.getEventById = async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      ...
      `SELECT
         events.*,
         COUNT(registrations.id)::int AS registrations_count
       FROM events
       LEFT JOIN registrations
       ON events.id = registrations.event_id
       WHERE events.id=$1
    ...
    if (derivedStatus !== event.status) {
      await pool.query(
        `UPDATE events SET status = $1 WHERE id = $2`,
        [derivedStatus, event.id]
      );
    ...
    if (event.status === 'deleted') {
      return res.status(403).json({
        message: "This event has been deleted and is no longer available for registration"
      });
    }
...
if (parking_enabled !== undefined) {
  updateFields.push(`parking_enabled=$${paramIndex++}`);
  updateValues.push(parking_enabled);
}
if (standard_slots !== undefined) {
  updateFields.push(`standard_slots=$${paramIndex++}`);
  updateValues.push(standard_slots);
}
if (premium_slots !== undefined) {
  updateFields.push(`premium_slots=$${paramIndex++}`);
  updateValues.push(premium_slots);
}
if (valet_slots !== undefined) {
  updateFields.push(`valet_slots=$${paramIndex++}`);
  updateValues.push(valet_slots);
}
if (standard_price !== undefined) {
  updateFields.push(`standard_price=$${paramIndex++}`);
  updateValues.push(standard_price);
}
if (premium_price !== undefined) {
  updateFields.push(`premium_price=$${paramIndex++}`);
  updateValues.push(premium_price);
}
if (valet_price !== undefined) {
  updateFields.push(`valet_price=$${paramIndex++}`);
  updateValues.push(valet_price);
}
...
exports.deleteEvent = async (req, res) => {
  ...
  try {
    ...
    /* BLOCK DELETE ONLY FOR LIVE EVENTS */
    
    if (event.status === "live") {
      return res.status(400).json({
        error: "Live events cannot be deleted",
        message: "Event is live. You can delete it only before it starts or after it is completed."
      });
    }
    ...
    const feedbackRecipientsResult = await pool.query(
      `SELECT DISTINCT
    ...
    const feedbackRecipients = feedbackRecipientsResult.rows || [];

    /* SOFT DELETE EVENT (PRESERVE REGISTRATIONS + FEEDBACK RELATION) */
    ...
    /* SEND FEEDBACK REQUEST EMAILS IN BACKGROUND */
    ...
    setImmediate(async () => {
      try {
        if (feedbackRecipients.length === 0) {
          console.log(`No feedback recipients found for event ${event.id}`);
          return;
        }

        const sendJobs = feedbackRecipients.map(async (participant) => {
          const participantEmail = String(participant.email || "").trim().toLowerCase();
          const participantName = participant.name || "Participant";
          const feedbackLink = `${frontendBaseUrl}/feedback-form/${event.id}?email=${encodeURIComponent(participantEmail)}`;
        ...
        console.log(`Feedback request emails finished for event ${event.id}: ${sentCount} sent, ${failedCount} failed`);
      } catch (mailError) {
        console.error(`Failed to send feedback emails for event ${event.id}:`, mailError);
      }
    });
    ...
    doc
      ...
      .fillColor("#9ca3af")
      .text(
        "Generated automatically by Evently Management System after event archival.",
        50,
        pageHeight - 60,
        { align: "center" }
      );
